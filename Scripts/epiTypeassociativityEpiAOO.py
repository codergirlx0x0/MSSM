...
#Corresponding Graphs: associativityAnalysis.pdf page 5
import pandas as pd
import csv
                            ...
                            for col4 in vals.columns:   
                                if col4=="Subject ID":
                                    ...
                                    for col2 in vals.columns:
                                        ...
                                        if col2=="Age at first seizure (approx) (Interview)":
                                            if not isNaN(vals[col2][ind]) and vals[col2][ind]!="Unknown":
                                                count+=1
                                                for col3 in vals.columns:
    ...
    if isinstance(subjects[i][0], str):
        ...
        if subjects[i][0]<=24.0:
            for j in subjects[i][1]:
                ...
                if j=="Lesional focal epilepsy":
                    lct+=1
NAFE.append(nct/len(subjects)*100)
LFE.append(lct/len(subjects)*100)
EE.append(ect/len(subjects)*100)
IGE.append(ict/len(subjects)*100)
nct=0
ict=0
ect=0
lct=0
...
ax.set_title('Epi AoO vs Epi Type N={}'.format(len(subjects)))
ax.set_ylim(8, 100)
