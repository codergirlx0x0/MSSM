...
#Corresponding Graphs: associativityAnalysis.pdf page 8
import pandas as pd
import csv
...
plt.legend()
plt.title('Epi AoO vs Verbality Status N={}'.format(len(t)))
plt.ylim(top=100)
plt.ylim(bottom=0)
...
plt.xticks([r + barWidth for r in range(len(verbal))], labels)
plt.legend()
plt.title('Epi AoO vs Verbality Status N={}'.format(len(s)))
plt.ylim(top=100)
plt.ylim(bottom=0)
