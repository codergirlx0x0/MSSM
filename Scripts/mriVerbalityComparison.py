...
#Corresponding graphs: mriAnalysis.pdf page 3
import pandas as pd
import csv
...
findValue("Nonverbal (Chart)")
findValue("Nonverbal (Self Assess)")
# if value=="renew":
#     value=findValue("Nonverbal (Self Assess)")
subjects2={}
for i in subjects:
...
for i in subjects2:
    ...
    if subjects2[i][0]=='Normal':
        ...
        if subjects[i][1]=="Yes":
            ...
            nct+=1
verbal.append(vct/len(subjects2)*100)
nonverbal.append(nct/len(subjects2)*100)
...
# verbal.append(vct/len(subjects2)*100)
# nonverbal.append(nct/len(subjects)*100)
...
fig, ax = plt.subplots()
rects1 = ax.bar(x - width/2, verbal, width, label='Verbal')
rects2 = ax.bar(x + width/2, nonverbal, width, label='Non Verbal')

# Add some text for labels, title and custom x-axis tick labels, etc.
ax.set_ylabel('%')
ax.set_title('MRI vs Verbality N={}'.format(len(subjects2)))
ax.set_xticks(x)
