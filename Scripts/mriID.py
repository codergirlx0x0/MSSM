#Purpose: Looks at the correlation between mri and ID by looking at percent of chart review complete population that has/doesn't have ID in each of the mri status categories
#Corresponding graphs: mriAnalysis.pdf page 1
import pandas as pd
import csv
...
fig, ax = plt.subplots()
rects1 = ax.bar(x - width/2, nodd, width, label='no ID')
rects2 = ax.bar(x + width/2, dd, width, label='ID')

# Add some text for labels, title and custom x-axis tick labels, etc.
ax.set_ylabel('%')
ax.set_title('MRI vs ID N={}'.format(len(subjects2)))
ax.set_xticks(x)
