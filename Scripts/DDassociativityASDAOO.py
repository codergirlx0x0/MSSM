#Purpose: Looks at the associativity between ASD Aoo and DD statuses and splits into two categories: DD status (3 types) percents fo Age 2 or less and dd percents for Age 2+
#Corresponding Graphs: associativityAnalysis.pdf page 2
import pandas as pd
import csv
...
plt.legend()
plt.title('ASD AoO vs DD Status N={}'.format(len(t)))
plt.ylim(top=100)
plt.ylim(bottom=0)
...
plt.xticks([r + barWidth for r in range(len(delay))], labels)
plt.legend()
plt.title('ASD AoO vs DD Status N={}'.format(len(s)))
plt.ylim(top=100)
plt.ylim(bottom=0)
